...
// Рассмотрим метод string.prototype trim() - он удаляет пробелы в начале и в конце строки!!!!
// И конечно же если вся строка состоит из пробелов, то псоле применения метода trim() она будет пустой, а вот пустая строка у нас
// проверяется внутри условия!
// Этот метод используется не только для prompt(), но и для любых других строк, которые мы получаем от пользователя или сервера

function rememberNyFilms() {
  for (let i = 0; i < 2; i++) {
    const a = prompt('Один из последних просмотренных фильмов?', '').trim(), // trim() удаляет пробелы в начале и в конце строки - вызывать разными спопобами, но лучше всего вызывать его сразу после prompt() и до проверки условия, что бы не проверять лишний раз пустую строку
